...
def process_tse(args, model, device, data_reader, output_wave_dir):
    ...
    with torch.no_grad():
        for videoPath in data_reader:  # Loop over all video samples
            ...
            video_args.savePath = os.path.join(output_wave_dir, savFolder.split(".")[0])
...
def main(video_args, args):
    # Initialization 
    print("-----video_args.savePath ", video_args.savePath)
    ...
    print("---------Scene detection for the video frames")
    scene = scene_detect(video_args)
    sys.stderr.write(time.strftime("%Y-%m-%d %H:%M:%S") + " Scene detection and save in %s \r\n" %(video_args.pyworkPath))	

    # Face detection for the video frames
    print("---------Face detection for the video frames")
    ...
    print("-----------combine files in pycrop")
    for idx, file in enumerate(files):
        print(file)
        
        # Convert AVI to MP4
        orig_mp4 = f"{file[:-9]}orig_{idx}.mp4"
        command = f"ffmpeg -i {file} {orig_mp4}"
        ...
        try:
            ...
            print(f"Error removing AVI file: {e}")
        
        # Remove WAV file
        try:
            wav_file = file.replace('.avi', '.wav')
            os.remove(wav_file)
        except OSError as e:
            print(f"Error removing WAV file: {e}")
        
        # Combine video with estimated audio
        est_wav = f"{file[:-9]}est_{idx}.wav"
        est_mp4 = f"{file[:-9]}est_{idx}.mp4"
        command = f"ffmpeg -i {orig_mp4} -i {est_wav} -c:v copy -map 0:v:0 -map 1:a:0 -shortest {est_mp4}"
        output = subprocess.call(command, shell=True, stdout=None)
        
        # Optionally remove estimated WAV file (currently commented out)
        # try:
        #     os.remove(est_wav)
        # except OSError as e:
        #     print(f"Error removing estimated WAV file: {e}")

    # Remove work directories
    try:
        shutil.rmtree(video_args.pyworkPath)
        shutil.rmtree(video_args.pyframesPath)
    except OSError as e:
...
def visualization(tracks, est_sources, video_args):
    ...
    for tidx, track in enumerate(tracks):
        ...
        # Second ffmpeg command (separated from remove command)
        ...
        # Remove the temporary file using os.remove instead of rm command
        try:
            os.remove(os.path.join(video_args.pyaviPath, f'video_out_{tidx}.avi'))
        except OSError as e:
    ...
    # Clean up remaining files using os.remove
    try:
        os.remove(os.path.join(video_args.pyaviPath, 'video_only.avi'))
        os.remove(os.path.join(video_args.pyaviPath, 'video.avi'))
        os.remove(os.path.join(video_args.pyaviPath, 'audio.wav'))
    except OSError as e:
